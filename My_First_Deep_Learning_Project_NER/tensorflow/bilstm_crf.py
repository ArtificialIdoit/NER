...
class Model:
    ...
    def build_net(self):
        ...
        with tf.variable_scope('bilstm+crf') as scope:
            ...
            self.viterbi_sequence, self.viterbi_score = tf.contrib.crf.crf_decode(full_connect,transition_params,tf.tile(np.array([self.sen_len]),np.array([self.batch_size])))
