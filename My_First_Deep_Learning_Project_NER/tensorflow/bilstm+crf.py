...
class Model:
    ...
    def build_net(self):
        self.input_data = tf.placeholder(tf.int32,shape = [self.config['batch_size'],self.config['sentence_len']],name = 'input_data')
        self.labels = tf.placeholder(tf.int32,shape = [self.config['batch_size'],self.config['sentence_len']],name = 'labels')
        #这个就是y_test吧？
        self.embedding = tf.placeholder(tf.float32,shape = tf.int32,shape = [self.config['vocabulary_size'],self.config['embedding_dim']],name = 'embedding')
        #name是在tensorflow命名空间中的名字，
        #而等号前面的则是在python命名空间下的临时指针，脚本运行完毕，这个空间会消失。
        #这块shape填None是不是太好，应该适当的加入维度检查。
        
        with tf.variable_scope('bilstm+crf') as scope:
        #提供一个命名域
            word_embeddings = tf.get_variables('word_embeddings',[self.config['vocabulary_size'],self.config['embedding_dim']])
            #这句与scope经常一同出现,防止重复构造
            #是不是所有的variables都是更新的对象？
            #tf.variables则每次都是构造新变量。
            if self.config['pretrained']:
                embedding_init = word_emeddings.assign(self.config['embeddings_pretrained'])
            ...
            lstm_forward_cell = tf.nn.rnn_cell.LSTMCell(self.config['emedding_dim'],forget_bias = 1.0,state_is_tuple=True)
            lstm_backforward_cell = tf.nn.rnn_cell.LSTMCell(self.config['embedding_dim'],forget_bias = 1.0,,state_is_tuple=True)
            ...
            (output_fw,output_bw),states = tf.nn.bidirectional_dynamic_rnn(lstm_forward_cell,
                                                                               lstm_backwwrd_cell,nput_embedded,
                                                                               dtype = tf.float32,
                                                                               time_major = False,
                                                                               scope = None)
            ...
            W = tf.get_variable(name='W',shape = [self.batch_size,2*self.config['embedding_dim'],self.config['tag_size']],dtype = tf.float32)
            #这几个维度怎么回事？解释一下？
            
            b = tf.get_variable(name='b',shape = [self.batch_size,self.config['sentence_len'],self.config['tag_size']],dtype = tf.float32)
            ...
            log_likelihood,transition_params = tf.contrib.crf.crf_log_likelihood(full_connect,self.labels,tf.tile(np.array([self.config['sen_len']]),np.array([self.config['batch_size']])))
            #返回值见api！！！
